...
void DirectXCommon::Initialize(WinApp* winApp_)
{
	...
	inputElementDescs[0].SemanticName = 0;
...
void DirectXCommon::PreDraw()
{
	...
	commandList->IASetVertexBuffers(0, 1, &triangle_->GetVertexBufferView());//VBVを設定
	//形状を設定。PSOに設定しているものとはまた別。同じものをせっていすると考えておけばいい
	commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	//描画!(DrawCall)。3頂点で1つのインスタンス。インスタンスについては今後
...
IDxcBlob* DirectXCommon::SetUpCompileShader(
	...
	LPCWSTR arguments[] = {
		...
		L"-ZI",L"Qemed_debug",//デバッグ用の情報を埋め込む
		L"Od"//最適化を外しておく
		L"-Zpr"//メモリレイアウトは行優先
	};
	//実際にShaderをコンパイルする
...
void DirectXCommon::SetUpRootSignature()
{
	...
	//シリアライズしてバイナリにする
	hr = D3D12SerializeRootSignature(&descriptionRootSignature, D3D_ROOT_SIGNATURE_VERSION_1, &signatureBlob, &errorblod);
	if (FAILED(hr)) {
...
void DirectXCommon::SetUpVertexResource(WinApp* winApp_)
{
	//頂点リソース用のヒープ設定
